...
void Custom::RobotControl() {
    ...
    // print imu info
    std::cout << motiontime << " " << state.imu.quaternion[0] << "w+" << state.imu.quaternion[1] << "x+"
              << state.imu.quaternion[2] << "y+" << state.imu.quaternion[3] << "z \n";

    for (auto i = 0; i < 12; i++) {
        std::cout << "motor:" << i << " motor.q = " << state.motorState[i].q << " motor.dq= " << state.motorState[i].dq
                  << std::endl;
    }
